...
/* package */ class ConcreteTable implements Table {
	 ...
	 *                            interface.
	 */
	// 3. select * 에서 addrId 가 2개 생기는 경우 해결
	public Table select(Selector where, Collection requestedColumns, Collection other) {
		...
		if (requestedColumns != null) // SELECT *
		{
			...
			while (column.hasNext()) 
				// 요청한 Table의 모든 컬럼 이름 받아오기 
				columnNames[i++] = column.next().toString();
			
			for(int k = 0; k < columnNames.length; k++) {
				// select 로 받은 문자열만 가지고 오는 것을 할 수 있음
				// 예) select street, first ~ 이면, select, street가 들어감
				System.out.println("columnNames "+ k + " : " + columnNames[k]);
			}
		}
		else 
		{
			...
			for(int i = 0; i < otherTables.length; i++) {
				try {
					ConcreteTable copy = (ConcreteTable)otherTables[i].clone();
					tempColumns = copy.columnNames;
				} catch (CloneNotSupportedException e) {
					e.printStackTrace();
				}
			}
			// Table의 모든 열 값들 붙이기
			String[] columns = new String[this.columnNames.length + tempColumns.length];
			System.arraycopy(this.columnNames, 0, columns, 0, this.columnNames.length);
			System.arraycopy(tempColumns, 0, columns, this.columnNames.length, tempColumns.length);
			
			// 열 값에 중복값이 있는지 확인하고 있으면 지우기 (굳이 필요 없을 듯)
			//columns= new HashSet<String>(Arrays.asList(columns)).toArray(new String[1]);
			
			// 최종 열 데이터 columnNames에 저장
			columnNames = columns;
		}
...
//				/* Failed correctly, do nothing */ }
		...
		public void testStore() throws IOException, ClassNotFoundException { // Flush the table to disk, then reread it.
			...
			Writer out = new FileWriter("people1.csv");
